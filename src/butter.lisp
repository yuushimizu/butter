...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (let ((test-functions-in-packages (make-hash-table :test 'equal)))
    ...
    (defun test-function (name)
      (or (gethash name (test-functions (symbol-package name)))
          (error 'named-test-not-found-error :test-name name)))
    (defun (setf test-function) (function name)
      (let* ((package (symbol-package name))
             (test-functions (or (gethash (package-name package) test-functions-in-packages)
