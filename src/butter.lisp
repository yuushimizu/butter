...
(defun call-as-test (name function)
  (handler-case
      (funcall function)
    (test-error-occurred (condition) (signal (make-condition 'test-error-occurred :test-name name :inner-condition condition)) nil)
    (error (condition) (signal (make-condition 'test-error-occurred :test-name name :occurred-error condition)) nil)))
	   ...
	   (lambda ()
	     (let ((,result% (catch ',result%
				 ...
				 (handler-case (progn ,@body)
				   (test-failed (,condition%) (fail nil ,condition%)))
				 nil))))
...
(define-test-type :function (function-name &rest arguments)
  (let ((argument-vars (mapcar (lambda (argument)
    ...
    `(let ,(remove nil (mapcar (lambda (var argument) (unless (keywordp var) (list var argument)))
       ...
       (handler-case (test t ,(call-form function-name argument-vars))
	 (test-failed () (fail (format nil "(~A~@{ ~S~}) is nil." ',function-name ,@argument-vars)))))))
(defmethod test-form-expand ((type symbol) arguments)
  (with-gensyms (condition%)
    `(with-default-success-handler
	 (handler-case (test :function ,type ,@arguments)
	   (test-failed (,condition%) (fail (message ,condition%)))))))
...
(defun test-function (name &optional (package *package*))
  (or (gethash name (test-functions package))
      (error (make-condition 'no-test-found :test-name name))))
